...
import { parseListStrategiesToolResult } from '@/strategy-resolution/mgmt-mcp-client'
...
describe('parseListStrategiesToolResult', () => {
  ...
  it('throws when there is no text content block', () => {
    expect(() => parseListStrategiesToolResult([])).toThrow(
  ...
  it('throws when the parsed body does not match the expected shape', () => {
    ...
    ).toThrow('malformed list_strategies response')
  })
})
