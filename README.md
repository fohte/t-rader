...
| `STRATEGY_AGENT_MODEL`   | 戦略 Agent の LLM 呼び出しに使うモデル名。実際に使用するモデル名は秘匿するため `.env` にはデフォルト値を置かない。未設定/空文字だと agent-config bundle 取得 (`GET /api/agent-configs/{purpose}/agent-config`) がエラーを返す。`.env.local` に設定すること | -                            |
| `WEB_SEARCH_MODEL`       | `search_web` MCP tool が使う web 検索対応モデル名。同上の理由で `.env` にはデフォルト値を置かない。未設定/空文字だと `search_web` 呼び出しがエラーを返す。`.env.local` に設定すること                                                                      | -                            |
| `GEMINI_MEDIA_MODEL`     | `query_media` MCP tool が使う動画/音声解析モデル名。同上の理由で `.env` にはデフォルト値を置かない。未設定/空文字だと `query_media` 呼び出しがエラーを返す。`.env.local` に設定すること                                                                    | -                            |
...
| 値        | 必要な追加変数                                                                                 | 用途                                                                                                                                                                                                                                                                                                                            |
| --------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `jquants` | `JQUANTS_API_KEY` (未設定時は DataProvider なしで起動)                                         | J-Quants API。契約範囲を問い合わせる専用 API が無いため、契約範囲外エラーのメッセージから実際の取得可能範囲を検出して追従する。検出が済むまでは範囲不明として扱い、無料プラン相当の範囲を暗黙に仮定して動作することはない。設定ページ (`/settings/jquants-plan`) から契約プランを手動指定でき、その場合は自動検出より優先される |
...
- [`docs/architecture.md`](./docs/architecture.md): backend の crate 構成、依存規則、境界の責務
