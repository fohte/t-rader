...
| 値        | 必要な追加変数                                                                                 | 用途                                                                                                                                                                                                                      |
| --------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `jquants` | `JQUANTS_API_KEY` (未設定時は DataProvider なしで起動)                                         | J-Quants API。契約範囲を問い合わせる専用 API が無いため、契約範囲外エラーのメッセージから実際の取得可能範囲を検出して追従する。検出が済むまでは範囲不明として扱い、無料プラン相当の範囲を暗黙に仮定して動作することはない |
