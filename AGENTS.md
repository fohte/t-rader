...
ローソク足チャート (Lightweight Charts)、ウォッチリストの API とテーブル、データプロバイダー抽象化は既存資産。ウォッチリストの frontend UI は削除済みで、backend の API とテーブルだけが残る。戦略 (= ワークスペース) ベースへの移行後に deprecate 予定。新規 UI は戦略起点で組む。
