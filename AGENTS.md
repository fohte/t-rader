...
ローソク足チャート (Lightweight Charts)、データプロバイダー抽象化は既存資産。新規 UI は戦略起点で組む。
...
- `backend/crates/core/application/` - agent task、indicator observation、kata-exec executor、LLM client、news aggregation、日足データソース (銘柄ごと / 全銘柄の日付指定)、銘柄マスタ、信用残、空売り、決算予定、バリュエーションの port と値型
