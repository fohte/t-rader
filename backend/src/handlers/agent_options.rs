        ...
        assert_eq!(
            ...
            serde_json::json!({
                "tools": [
                    ...
                    {"name": "query_data", "description": "Fetch daily OHLCV bars for one or more instruments over a shared date range from the DB. Results are in the same order as instrument_ids; an instrument with no ingested data returns an empty bars array rather than an error."},
