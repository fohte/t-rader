...
#[utoipa::path(
    ...
    responses(
        (status = 200, body = AgentGraphBody),
        (status = 400, description = "YAML が不正、または for_each の参照先が実在しない", body = ErrorResponse),
        (status = 404, body = ErrorResponse),
        (status = 422, body = ErrorResponse),
        (status = 500, body = ErrorResponse),
    )
...
mod tests {
    ...
    async fn agent_graph_defaults_to_empty_content(pool: PgPool) {
        ...
        let get = server
            ...
            .await;
        get.assert_status_ok();
        assert_eq!(get.json::<serde_json::Value>(), json!({ "content": "" }),);
    }
    ...
    async fn put_then_get_agent_graph_round_trips(pool: PgPool) {
        ...
        let put = server
            ...
            .await;
        put.assert_status_ok();
        assert_eq!(put.json::<serde_json::Value>(), json!({ "content": yaml }),);

        let get = server
            .get(&format!("/api/strategies/{id}/agent-graph"))
            .await;
        get.assert_status_ok();
        assert_eq!(get.json::<serde_json::Value>(), json!({ "content": yaml }));
    }
