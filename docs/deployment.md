...
| 条件付き                       | `JQUANTS_API_KEY`        | `DATA_PROVIDER=jquants` 時のみ必要。取得範囲は無料枠既定 (12 週遅延・2 年分の履歴) から始まり、契約範囲外エラーのレスポンスから実際の契約範囲を自動学習して追従する | DataProvider なしで起動 (`query_data` が動かない)                                                           |
