...
| 必須 (in-cluster / 外部公開時) | `MCP_ALLOWED_HOSTS`      | MCP server が受理する `Host` ヘッダの追加許可リスト (カンマ区切り)。in-cluster Service DNS 名に加え、`/mcp/mgmt` を外部公開する場合は公開ホスト名も含める | 該当する呼び出し元からの接続が 403 で弾かれる ([`docs/mcp.md`](./mcp.md))                                                                               |
...
| 条件付き                       | `JQUANTS_API_KEY`        | `DATA_PROVIDER=jquants` 時に使用する J-Quants API キー。設定する場合は `JQUANTS_PLAN` も必要                                                              | 未設定なら DataProvider なしで起動 (`read_portfolio` / `check_buyable_qty` の現在値取得が動かない。`query_data` は DB 読み取りのみのため影響を受けない) |
| 条件付き                       | `JQUANTS_PLAN`           | J-Quants の契約プラン (`free` / `light` / `standard` / `premium`)                                                                                         | `JQUANTS_API_KEY` 設定時に未設定または不正な値なら起動失敗。API キー未設定時は DataProvider なしで起動                                                  |
