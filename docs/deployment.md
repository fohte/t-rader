...
| 条件付き                       | `JQUANTS_API_KEY`        | `DATA_PROVIDER=jquants` 時のみ必要。契約範囲を問い合わせる専用 API が無いため、契約範囲外エラーのメッセージから実際の取得可能範囲を学習して追従する (学習が済むまでは範囲不明として扱い、無料プラン相当の範囲を仮定して黙って動作することはない) | DataProvider なしで起動 (`query_data` が動かない)                                                           |
