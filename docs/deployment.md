...
- backend Service への外部からの経路は環境に応じて構成する (例: `/mcp/mgmt` のみ Ingress/Tunnel で外部公開し、他は frontend 経由に閉じる)。
...
| 必須              | `MCP_ALLOWED_HOSTS`      | MCP server が受理する `Host` ヘッダの追加許可リスト (カンマ区切り)。in-cluster Service DNS 名に加え、`/mcp/mgmt` を外部公開する場合は公開ホスト名も含める | 該当する呼び出し元からの接続が 403 で弾かれる ([`docs/mcp.md`](./mcp.md))                                   |
