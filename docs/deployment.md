...
| 必須 (production) | `TRADER_AGENT_API_URL`   | t-rader-agent internal API の base URL                                                                                                           | 未設定 (`disabled` 以外の値なし) は起動失敗。`disabled` を明示すると warning ログを出して agent task client を無効化して起動 |
