...
| 条件付き                       | `JQUANTS_API_KEY`        | `DATA_PROVIDER=jquants` 時に使用する J-Quants API キー。設定する場合は provider の選択にかかわらず `JQUANTS_PLAN` も必要                                  | 未設定なら DataProvider なしで起動 (`read_portfolio` / `check_buyable_qty` の現在値取得が動かない。`query_data` は DB 読み取りのみのため影響を受けない) |
| 条件付き                       | `JQUANTS_PLAN`           | J-Quants の契約プラン (`free` / `light` / `standard` / `premium`)                                                                                         | API キー設定時に未設定または不正な値なら起動失敗。API キー未設定時は無視する                                                                            |
