...
| `list_watch_targets` | `strategy_id`, `limit?`                                                                         | 人間が `origin=human` で登録した監視対象銘柄 (`ref_kind=stock`, `status=active`) を古い順に返す。エージェント自身が追加した関心 (`origin=llm`) や `status=archived` のものは含まない。保有状況によるフィルタは行わないため、除外したい場合は `read_portfolio` / `check_buyable_qty` と組み合わせる                                                                                                                                                                                                                     |
