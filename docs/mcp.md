...
| `read_news`          | `strategy_id`, `limit?`                                                                         | 戦略に紐づいた未読ニュースを古い順に返す (`has_more` で続きの有無を示す)。同一戦略・stream 単位の checkpoint を呼び出しごとに自動で進めるため、実行間隔が空いても取りこぼさない。1 記事が複数 interest に一致していれば一致ごとに 1 行になる (url が重複し得る)                                                                                                                                                                                                            |
