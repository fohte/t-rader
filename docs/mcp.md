...
| `write_note`        | `strategy_id`, `note_id?`, `title?`, `body_md?`, `type_tag?`, `frontmatter_json?`               | ノートを作成または更新。`created_by_kind` はサーバー側で `"llm"` 固定。更新時は status を unread に戻す                                                                                             |
...
| `read_comments`     | `strategy_id`, `target_kind` (`note` / `annotation`), `target_id`, `resolved?`                  | 対象に付いたレビューコメント一覧 (古い順)。スレッドは `parent_id` で表現。`resolved` で絞り込み可。各コメントは `anchor_text`/`start_line`/`end_line`/`drifted` を含み、note 本文中の位置を追跡する |
