...
| `write_note`        | `strategy_id`, `note_id?`, `title?`, `body_md?`, `type_tag?`, `frontmatter_json?`, `graphs?`    | ノートを作成または更新。`created_by_kind` はサーバー側で `"llm"` 固定。更新時は status を unread に戻す。`graphs` は配列ごと置換 (id 単位の部分更新は不可)、省略時は既存の図を変更しない |
