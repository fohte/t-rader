...
- 例外が 3 つある。`read_portfolio` は、戦略は口座内のお金の区分に過ぎず分析は口座全体を見る、という設計上、口座全体の集計には `x-strategy-id` をスコープとして使わない (ただし接続元戦略自身のスライス (投資可能額を含む) を追加で返すため、ヘッダの値もその選択に使う)。`search_refs` は stock/indicator/sector/theme が戦略に属さないマスタデータであるため、`x-strategy-id` をそもそも検索条件に使わない。`search_news` も news_item 全体を検索対象にするため、`x-strategy-id` を検索条件に使わない。

任意ヘッダ `x-execution-id` で実行単位を識別できる。値の単位は t-rader-agent 側の実行経路で異なる: agent_graph 実行ではフェーズ / for_each 要素ごとの実行ステップ 1 件 (`{a2a_task_id}:{step_id}`)、agent_graph 未設定の単一 invoke では `runStrategyAgent` 呼び出し全体 (A2A タスクの `taskId` そのもの) が単位になる。戦略境界の検査対象ではなく、`write_note` の冪等性や `read_news` の checkpoint 記録 (下表参照) にのみ使う。
...
| `write_note`                | `strategy_id`, `note_id?`, `title?`, `body_md?`, `type_tag?`, `frontmatter_json?`, `graphs?`           | ノートを作成または更新。`created_by_kind` はサーバー側で `"llm"` 固定。更新時は status を unread に戻す。`graphs` (`GraphDef` の配列、定義は `backend/src/services/graph.rs`) は配列ごと置換 (id 単位の部分更新は不可)。省略時は既存の図を変更しない。`note_id` 省略時、`x-execution-id` ヘッダがあれば同一値の既存ノートを探して更新し、複数の create 呼び出しが同一ノートへ収束する (idempotent。値の単位は上記「戦略境界の保証」節を参照)                               |
...
| `create_annotation`         | `strategy_id`, `target_symbol`, `target_kind`, `timestamp`, `text`, `price?`, `linked_note_id?`        | アノテーション作成。`target_kind` は自由記述の分類ラベル                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `search_web`                | `strategy_id`, `query`                                                                                 | 問い合わせ文を web 検索対応モデルに渡し、テキストと出典 URL (`citations`, 重複除去済み) を返す。モデルは既定で ChatGPT Plus 経由 (`chatgpt/gpt-5.6-luna`)、`WEB_SEARCH_MODEL` 環境変数で上書き可 (Gemini 等)。戦略タスク実行 (`x-execution-id` の `a2a_task_id` 部分) あたり 20 回まで、超過時は LiteLLM を呼ばずに reject する                                                                                                                                            |
...
| `read_news`                 | `strategy_id`, `limit?`                                                                                | 戦略に紐づいた未読ニュースを古い順に返す (`has_more` で続きの有無を示す)。同一戦略と stream 単位の checkpoint を呼び出しごとに自動で進めるため、実行間隔が空いても取りこぼさない。1 記事が複数の関心に一致していれば一致ごとに 1 行になる (`url` が重複し得る)                                                                                                                                                                                                             |
| `search_news`               | `strategy_id`, `keyword?`, `from?` (YYYY-MM-DD), `to?` (YYYY-MM-DD), `limit?`                          | `news_item` を `title`/`body_snippet` の部分一致 (大文字小文字を区別しない) と `published_at` の期間 (共に inclusive) で直接検索し、新しい順に返す。`news_strategy_link` を経由しないため、`strategy_interest` に語が登録されているかどうかに結果は左右されない (`read_news` が引き継いでいた制約を回避する)                                                                                                                                                               |
| `search_refs`               | `strategy_id`, `query`, `limit?`                                                                       | 一級参照型 (`stock`/`indicator`/`sector`/`theme`) を `id`/`name` の部分一致 (大文字小文字を区別しない) で横断検索し、`ref_kind`/`ref_id`/`name` を `name` 順で返す。4 テーブルを `UNION ALL` した raw SQL で実装 (umbrella テーブルは作らない)。結果はそのまま `add_interest` の入力に使える                                                                                                                                                                               |
