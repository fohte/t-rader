...
| `check_buyable_qty`  | `strategy_id`, `symbol`                                                                         | 指定銘柄をあと何株買えるかを、`strategy.risk_policy.max_position_ratio` / `account_risk_policy.max_sector_ratio` / 戦略の未使用投資可能額の 3 制約それぞれについて算出し、全体の最小値 (`max_qty`) とどの制約が効いたか (`binding_constraint`) を返す。保有していない銘柄でも `current_qty=0` として計算する。各上限株数は 100 株単位 (`lot_size`) に切り捨てる。制約が未設定なら `status=unlimited`、価格やセクターなど計算に必要な値が欠けていれば `status=unavailable` (`reason` 付き) を返し、誤った数値は返さない |
