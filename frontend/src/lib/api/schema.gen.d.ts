...
export interface components {
  schemas: {
    ...
    StrategyTaskStatusResponse: {
      ...
      /**
       * Format: date-time
       * @description 実行の論理的な基準時刻。監査用に記録し、agent がプロンプトにも含めて LLM に伝える。
    ...
    /**
     * @description `GET /api/strategies/:id/tasks` と `GET /api/tasks` の一覧要素。`steps`/`result_text`
     *     は一覧では使わないため含めない。
     */
    StrategyTaskSummary: {
      /**
      ...
      phase: string
      prompt: string
      /** @description 投入時に指定された purpose。`None` は purpose カラム追加前に作成された行に限られる。 */
      purpose?: string | null
      source: string
      /** Format: uuid */
      strategy_id: string
